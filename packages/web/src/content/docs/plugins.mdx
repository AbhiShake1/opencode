...
OpenCode plugins are **server-side** modules. They run inside the OpenCode runtime (the same process that powers `opencode serve`), and they receive events from the internal bus. The TUI is a client of that server, so **there is no separate “TUI plugin” type**. Instead, TUI behavior is surfaced as **`tui.*` bus events** that your plugin can listen to via the `event` hook.

In short:

- **Server vs TUI**: there is a single plugin system. TUI-specific functionality is delivered through `tui.*` events.
- **When TUI events fire**: only when a TUI is running (or when a client drives the TUI via `/tui/*` endpoints).
- **Where plugins execute**: in the server process, not inside the TUI UI renderer.
